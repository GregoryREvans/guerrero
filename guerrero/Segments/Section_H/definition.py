...
rmaker_one = abjadext.rmakers.TaleaRhythmMaker(
    talea=abjadext.rmakers.Talea(counts=[1, 1, 1, 5, 3, 2, 4], denominator=8),
...
rmaker_two = abjadext.rmakers.TaleaRhythmMaker(
    talea=abjadext.rmakers.Talea(counts=[4, 3, 1, 5, 2], denominator=8),
    beam_specifier=abjadext.rmakers.BeamSpecifier(
        beam_divisions_together=True, beam_rests=False
    ),
...
open(f"{build_path}/Section_H.ly", "w").writelines(score_lines[15:-1])
...
for count, staff in enumerate(abjad.iterate(score).components(abjad.Voice)):
    ...
    open(f"{build_path}/Section_H.ly", "w").writelines(part_lines[15:-1])
time_6 = time.time()
