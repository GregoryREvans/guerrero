...
markup = abjad.Markup(r"\markup \bold { E }",  )
...
open(f"{build_path}/Section_E.ly", "w").writelines(score_lines[15:-1])
...
for count, staff in enumerate(abjad.iterate(score).components(abjad.Voice)):
    ...
    open(f"{build_path}/Section_E.ly", "w").writelines(part_lines[15:-1])
time_6 = time.time()
